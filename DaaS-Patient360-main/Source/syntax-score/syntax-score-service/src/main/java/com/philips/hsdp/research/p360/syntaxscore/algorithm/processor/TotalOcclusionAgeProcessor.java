 ...
 */
@Service
public class TotalOcclusionAgeProcessor extends SyntaxScoreProcessor {
	...
	/** 
	 * This function is used to calculate Syntax score left dominance for Total Occlusion Age
	 * 
	...
	public void calculateSyntaxScoreLeftDominance() {
		...
		if (syntaxScoreCodeableConceptCodeValue.getAgeOfTotalOcclusion() != null) {
			if (syntaxScoreCodeableConceptCodeValue.getAgeOfTotalOcclusion() == 0) {
				tempSyntaxScore += TotalOcclusionConstantLeft.L_TOTAL_OCCLUSION_AGE_NO;
				totalSyntaxScore.put("Age T.O. is no ", TotalOcclusionConstantLeft.L_TOTAL_OCCLUSION_AGE_NO);
			} else if (syntaxScoreCodeableConceptCodeValue.getAgeOfTotalOcclusion() == 1) {
				tempSyntaxScore += TotalOcclusionConstantLeft.L_TOTAL_OCCLUSION_AGE_YES;
				totalSyntaxScore.put("Age T.O. is yes ", TotalOcclusionConstantLeft.L_TOTAL_OCCLUSION_AGE_YES);
			} else if (syntaxScoreCodeableConceptCodeValue.getAgeOfTotalOcclusion() == 2) {
				tempSyntaxScore += TotalOcclusionConstantLeft.L_TOTAL_OCCLUSION_AGE_UNKNOWN;
				totalSyntaxScore.put("Age T.O. is unknown ", TotalOcclusionConstantLeft.L_TOTAL_OCCLUSION_AGE_UNKNOWN);
			}
	...
	public void calculateSyntaxScoreRightDominance() {
		...
		if (syntaxScoreCodeableConceptCodeValue.getAgeOfTotalOcclusion() != null) {
			if (syntaxScoreCodeableConceptCodeValue.getAgeOfTotalOcclusion() == 0) {
				tempSyntaxScore += TotalOcclusionConstantRight.R_TOTAL_OCCLUSION_AGE_NO;
				totalSyntaxScore.put("Age T.O. is no ", TotalOcclusionConstantRight.R_TOTAL_OCCLUSION_AGE_NO);
			} else if (syntaxScoreCodeableConceptCodeValue.getAgeOfTotalOcclusion() == 1) {
				tempSyntaxScore += TotalOcclusionConstantRight.R_TOTAL_OCCLUSION_AGE_YES;
				totalSyntaxScore.put("Age T.O. is yes ", TotalOcclusionConstantRight.R_TOTAL_OCCLUSION_AGE_YES);
			} else if (syntaxScoreCodeableConceptCodeValue.getAgeOfTotalOcclusion() == 2) {
				tempSyntaxScore += TotalOcclusionConstantRight.R_TOTAL_OCCLUSION_AGE_UNKNOWN;
				totalSyntaxScore.put("Age T.O. is unknown ", TotalOcclusionConstantRight.R_TOTAL_OCCLUSION_AGE_UNKNOWN);
			}
			syntaxScoreRequest.setSyntaxScore(tempSyntaxScore);
			syntaxScoreRequest.setTotalSyntaxScore(totalSyntaxScore);
		}
	 ...
	 * calculate Syntax score Left Dominance and Right Dominance for Total Occlusion Age 
	 * 
	 * @inheritDoc     : override super class SyntaxScoreProcessor.
