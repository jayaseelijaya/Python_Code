...
/* @Author Sunil Kumar*/
@Service
public class BifurcationProcessor extends SyntaxScoreProcessor {
	...
	/** 
	 * This function is used to calculate Syntax score left dominance for Bifurcation
	 * 
	...
	public void calculateSyntaxScoreLeftDominance() {
		...
		} else if (!(syntaxScoreCodeableConceptCodeValue.isBifurcationValue())) {
			super.setSkipCurrentProcessor(2);
		}
	}

	/** 
	 * This function is used to calculate Syntax score Right dominance for Bifurcation
	 * 
	 * @inheritDoc : override super class SyntaxScoreProcessor.
	 * @param float                           : to generate score in float value
	 * @param treeMap<String,float>           : get and set string and float value for syntax score in treeMap
	...
	public void calculateSyntaxScoreRightDominance() {
		float tempSyntaxScore = syntaxScoreRequest.getSyntaxScore();
		if (syntaxScoreCodeableConceptCodeValue.isBifurcationValue()) {
			super.setSkipCurrentProcessor(true);
		} else if (!(syntaxScoreCodeableConceptCodeValue.isBifurcationValue())) {
			super.setSkipCurrentProcessor(2);
