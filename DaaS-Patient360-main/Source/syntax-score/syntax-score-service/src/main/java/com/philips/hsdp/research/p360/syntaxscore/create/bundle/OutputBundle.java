...
public class OutputBundle {
	 ...
	 */
	public Bundle createBundle(List<String> subScoreObsList, List<Observation> observationList, Patient patientResource) {
		...
		Reference reference = new Reference(patientResource);
		bundleEntryComponent.setFullUrl(patientResource.getId());
		bundleEntryComponent.setResource(patientResource);
		BundleEntryRequestComponent bundleEntryRequestComponentPt = new BundleEntryRequestComponent();
		bundleEntryRequestComponentPt.setUrl(patientResource.getResourceType().toString());
		bundleEntryRequestComponentPt.setMethod(value);
		bundleEntryComponent.setRequest(bundleEntryRequestComponentPt);
		...
		for (Observation observation : parsedList) {
			...
			//For request-method
			bundleEntryRequestComponent.setUrl(observation.getResourceType().toString());
