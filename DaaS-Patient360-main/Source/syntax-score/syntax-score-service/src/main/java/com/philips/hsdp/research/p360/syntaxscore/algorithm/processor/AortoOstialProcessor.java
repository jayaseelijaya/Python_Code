...
import com.philips.hsdp.research.p360.syntaxscore.constant.AortoOstialConstantLeft;
import com.philips.hsdp.research.p360.syntaxscore.constant.AortoOstialConstantRight;

/* @Author Sunil Kumar*/
@Service
public class AortoOstialProcessor extends SyntaxScoreProcessor {
	...
	/** 
	 * This function is used to calculate Syntax score left dominance for Aorto Ostial
	 * 
	...
	public void calculateSyntaxScoreLeftDominance() {
		...
		if (syntaxScoreCodeableConceptCodeValue.isAortoOstialValue()) {
			tempSyntaxScore += AortoOstialConstantLeft.L_AORTOOSTIAL_VALUE_YES;
			totalSyntaxScore.put("Aorto Ostial lesion ", AortoOstialConstantLeft.L_AORTOOSTIAL_VALUE_YES);
		} else if (!(syntaxScoreCodeableConceptCodeValue.isAortoOstialValue())) {
			tempSyntaxScore += AortoOstialConstantLeft.L_AORTOOSTIAL_VALUE_NO;
		}
	...
	/** 
	 * This function is used to calculate Syntax score Right dominance for Aorto Ostial
	 * 
	 * @inheritDoc : override super class SyntaxScoreProcessor.
	 * @param float                           : to generate score in float value
	 * @param treeMap<String,float>           : get and set string and float value for syntax score in treeMap
	...
	public void calculateSyntaxScoreRightDominance() {
		...
		totalSyntaxScore = syntaxScoreRequest.getTotalSyntaxScore();
		if (syntaxScoreCodeableConceptCodeValue.isAortoOstialValue()) {
			tempSyntaxScore += AortoOstialConstantRight.R_AORTOOSTIAL_VALUE_YES;
			totalSyntaxScore.put("Aorto Ostial lesion ", AortoOstialConstantRight.R_AORTOOSTIAL_VALUE_YES);
		} else if (!(syntaxScoreCodeableConceptCodeValue.isAortoOstialValue())) {
			tempSyntaxScore += AortoOstialConstantRight.R_AORTOOSTIAL_VALUE_NO;
		}
		syntaxScoreRequest.setSyntaxScore(tempSyntaxScore);
		syntaxScoreRequest.setTotalSyntaxScore(totalSyntaxScore);
	}
	 ...
	 * calculate Syntax score Left Dominance and Right Dominance for Aorto Ostial 
	 * 
	 * @inheritDoc     : override super class SyntaxScoreProcessor.
