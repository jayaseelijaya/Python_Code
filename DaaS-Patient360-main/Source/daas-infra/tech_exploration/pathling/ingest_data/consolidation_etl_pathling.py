...
while True:
    for msg in consumer:
        if isinstance(msg.value, bytes):
            ...
            resource_type = msg_response['resourceType']
            if resource_type == 'Bundle':
                response = requests.post(url, headers = headers, json = msg_response)
                print(response.status_code)
            else:
                id = msgResponse['id']
                response = requests.put(url+resource_type+"/"+id, headers = headers, json = msg_response)
                print(response.status_code)