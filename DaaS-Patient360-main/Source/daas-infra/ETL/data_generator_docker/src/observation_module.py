...
Observation_folder = observation_dir +'/Dataset/'
...
def observation(patient_newname,new_patient_id,pract_id):
    ...
    for observation_vital_sign in range(0,4): 
        ...
        with open(observation_dir + '/Sample_Template/Observation_vital_signs.json') as dataset:
        ...
        with open(Observation_folder + filename, mode='w+', encoding='utf-8') as feedsjson:
    ...
    for observation_lab in range(0,2): 
        with open(observation_dir + '/Sample_Template/Observation_laboratory.json') as dataset:
            Observation_id = uuid.uuid4()
            jsonData = json.load(dataset)
            oldid = jsonData['id']
            jsonData['id'] = str(Observation_id)
        ...
        with open(observation_dir + '/Sample_Template/Observation_blood_pressure.json') as dataset:
            Observation_id = uuid.uuid4()
            jsonData = json.load(dataset)
            oldid = jsonData['id']
            jsonData['id'] = str(Observation_id)
            ...
            #resourceJson = jsonData.copy()
            #jsonData['resourceJson'] = json.dumps(resourceJson)
        filename1  = "Observation_blood_pressure"
        filename2  = ".json"
        filename   = filename1 + str(observation_lab) + filename2 
        with open(Observation_folder + filename, mode='w+', encoding='utf-8') as feedsjson:
            feedsjson.write(json.dumps(jsonData))
            feedsjson.close()
            print(filename)
    ...
    for observation_lab in range(0,2): 
        with open(observation_dir + '/Sample_Template/Observation.json') as dataset:
            Observation_id = uuid.uuid4()
            jsonData = json.load(dataset)
            oldid = jsonData['id']
            jsonData['id'] = str(Observation_id)
            ...
            jsonData['issued'] = '20' + str((r.randint(10,18))) + '-0' + str((r.randint(1,9))) + '-0' + str((r.randint(1,9))) + 'T03:30:10+01:00'
        filename1  = "Observation_laboratory"
        filename2  = ".json"
        filename   = filename1 + str(observation_lab)+"_CO2" + filename2 
        with open(Observation_folder + filename, mode='w+', encoding='utf-8') as feedsjson:
            feedsjson.write(json.dumps(jsonData))
            feedsjson.close()
            print(filename)
