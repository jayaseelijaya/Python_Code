 ...
 */
@Repository
public class PrestoDAOImpl implements BaseDAO {
	private static final Logger log = LoggerFactory.getLogger(PrestoDAOImpl.class);
	private String emrinstancepresto = System.getProperty("emr_instance_presto");

	@Override
	public ResponseDTO getDataForResourceR4(String id, String tableName)
		...
		try {
			...
			con = DriverManager.getConnection(emrinstancepresto, "hadoop", "");
			...
			String query = "select resourcejson from " + tableName + " where id = '" + id + "'";
			...
			while (rs.next()) {
				...
				resourceList.add(jsonResponseForResourceJson);
