...
with open(home_path + '/Test/Automation/python/syntax_score_standalone_test/invalid_syntaxscore_bundle.json','r') as dataset:
...
test_result = {
    "Test Name" : "Verification of negative test case for Syntaxscore calculator API",
    "Test Discription" : "Verify the negative test case for Syntaxscore calculator API with invalid data",
    "Test Result" : "",
    "Start Time" : "",
    "End Time" : "",
    "Total Time" : ""
...
try:
    ...
    response = requests.request(method = "POST", url = syntaxscore_calculator, headers = post_call_header, json = jsondata)
    text = response.text
    print(text)
    if response.status_code == 500:
        print("Verification of negative test case for Syntaxscore calculator API is passed ")
        test_result['Test Result'] = "Pass"
    else:
        print("Verification of negative test case for Syntaxscore calculator API is Failed ")
        test_result['Test Result'] = "Fail"

except:
    print("Verification of negative test Syntaxscore calculator is failed. Exception raised")
    test_result['Test Result'] = "Fail"
